...
for i=1:length(allDsmfld)    
    ...
    for j=1:length(fld)     
        ...
        csvArray(u+j,3) = outK.(fld{j}).dsmDV;
        csvArray(u+j,4) = outK.(fld{j}).levOrbitTime;
        csvArray(u+j,5) = outK.(fld{j}).vinflaunch;
        csvArray(u+j,6) = outK.(fld{j}).vinf1(1);
        csvArray(u+j,7) = outK.(fld{j}).vinf1(2);
        csvArray(u+j,8) = outK.(fld{j}).vinf2(1);
        csvArray(u+j,9) = outK.(fld{j}).vinf2(2);
        csvArray(u+j,10) = outK.(fld{j}).bendingdeg;
        csvArray(u+j,11) = outK.(fld{j}).r_car;
        csvArray(u+j,12) = outK.(fld{j}).dvegaRA;
    end
    u = u+j;
end   
% Writing Table and Exporting to .csv
csvTable = array2table(csvArray,'VariableNames', ...
    {'K','Theta','DSMDV','dt','vinfLaunch','vinf1x','vinf1y', ...
